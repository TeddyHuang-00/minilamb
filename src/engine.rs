...
/// let identity = Expr::Abs(Box::new(Expr::Var(0)));
/// let arg = Expr::Var(1); // Free variable y
/// let app = Expr::App(Box::new(identity), Box::new(arg.clone()));
///
/// let result = reduce_once(&app).unwrap();
/// // After substitution and shift, Var(1) becomes Var(0)
/// assert_eq!(result, Some(Expr::Var(0)));
/// ```
    ...
    let result = match expr {
        ...
        Expr::App(func, arg) => {
            if let Expr::Abs(body) = func.as_ref() {
                // Perform substitution: body[0 := arg]
                let substituted = substitute(0, arg, body)?;
...
/// // After evaluation, Var(1) becomes Var(0)
...
mod tests {
    ...
    fn test_reduce_once_identity_application() {
        // (λx.x) y → y
        // After substitution and shift down, y (which was free var 1) becomes var 0
        let identity = Expr::abs(Expr::var(0));
        let arg = Expr::var(1);
        let app = Expr::app(identity, arg);

        let result = reduce_once(&app).unwrap();
        // The result should be var(0) since free variable gets shifted down
        assert_eq!(result, Some(Expr::var(0)));
    }
    ...
    fn test_reduce_once_const_function() {
        ...
        // After substitution and shift, arg1 (var 2) becomes var 3 in the abstraction
        // body
        let expected = Expr::app(Expr::abs(Expr::var(3)), Expr::var(3));
    ...
    fn test_reduce_once_nested_abstraction() {
        ...
        // variable
        let inner_app = Expr::app(Expr::abs(Expr::var(1)), Expr::var(0));
        let outer_abs = Expr::abs(inner_app);

        let result = reduce_once(&outer_abs).unwrap();
        // After substitution and shift, the free variable z (index 1) becomes index 0
        let expected = Expr::abs(Expr::var(0));
        assert_eq!(result, Some(expected));
    }
    ...
    fn test_reduce_once_application_order() {
        ...
        let self_app = Expr::abs(Expr::app(Expr::var(0), Expr::var(0)));
        let identity = Expr::abs(Expr::var(0));
    ...
    fn test_evaluate_identity() {
        // (λx.x) y → y
        // After evaluation, free variable y gets shifted down to var(0)
        let identity = Expr::abs(Expr::var(0));
        let arg = Expr::var(1);
        let app = Expr::app(identity, arg);
    ...
    fn test_evaluate_church_true() {
        ...
        let arg1 = Expr::var(2);
        let arg2 = Expr::var(3);
        let app1 = Expr::app(church_true, arg1.clone());
        let app2 = Expr::app(app1, arg2);
    ...
    fn test_evaluate_church_false() {
        ...
        let church_false = Expr::abs(Expr::abs(Expr::var(0)));
        let arg1 = Expr::var(2);
        let arg2 = Expr::var(3);
        ...
        let result = evaluate(&app2, 100).unwrap();
        // After two β-reductions and shifts, var(3) becomes var(2)
    ...
    fn test_evaluate_reduction_limit_exceeded() {
        ...
        let omega_term = Expr::abs(Expr::app(Expr::var(0), Expr::var(0)));
    ...
    fn test_evaluate_complex_reduction() {
        ...
        let church_two = Expr::abs(Expr::abs(Expr::app(
            Expr::var(1),
            Expr::app(Expr::var(1), Expr::var(0)),
        )));
        let const_func = Expr::abs(Expr::abs(Expr::var(1)));
        let arg1 = Expr::var(2);
        let arg2 = Expr::var(3);
        ...
        // After multiple reductions and shifts, this should result in an abstraction
        ...
        match result {
            Expr::Abs(_) | Expr::Var(_) => (), // Accept either result
            Expr::App(..) => panic!("Unexpected result type"),
        }
    }
    ...
    fn test_y_combinator_fixed_point() {
        ...
        let y_combinator = Expr::abs(Expr::app(
            Expr::abs(Expr::app(
                Expr::var(1),
                Expr::app(Expr::var(0), Expr::var(0)),
            )),
            Expr::abs(Expr::app(
                Expr::var(1),
                Expr::app(Expr::var(0), Expr::var(0)),
